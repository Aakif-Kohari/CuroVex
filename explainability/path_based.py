...
class ExplanationPath:
    ...
    edges: list[PathEdge]
    meta_path_pattern: (
        str  # e.g. "Drug -[TARGETS]-> Protein -[ASSOCIATED_WITH]-> Disease"
    )
...
def find_paths(
    ...
    with driver.session() as session:
        ...
        for record in result:
            ...
            ep = ExplanationPath(
                nodes=path_nodes, edges=path_edges, meta_path_pattern=""
            )
...
def format_explanation(
    explanations: list[PathExplanation], drug_id: int, disease_id: int
) -> str:
...
def main():
    parser = argparse.ArgumentParser(
        description="Path-based explanations for drug-disease predictions."
    )
    ...
    parser.add_argument(
        ...
        default=3,
        choices=[1, 2, 3],
        help="Maximum number of hops (default: 3)",
    )
