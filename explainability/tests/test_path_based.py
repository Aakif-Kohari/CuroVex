...
class TestExtractMetaPathPattern:
    def test_two_hop_pattern(self):
        ...
        edges = [PathEdge(0, 1, "TARGETS"), PathEdge(1, 2, "ASSOCIATED_WITH")]
        path = ExplanationPath(nodes, edges, "")
        assert (
            extract_meta_path_pattern(path)
    ...
    def test_single_hop_pattern(self):
        nodes = [PathNode(0, "D", ["Drug"]), PathNode(1, "Dis", ["Disease"])]
        edges = [PathEdge(0, 1, "TREATS")]
        path = ExplanationPath(nodes, edges, "")
...
class TestGroupByPattern:
    ...
    def test_sorts_by_support(self, sample_explanation_paths):
        ...
        assert (
            explanations[0].meta_path_pattern
            == "Drug -[TARGETS]-> Gene -[ASSOCIATED_WITH]-> Disease"
        )

    def test_different_patterns_separate(self, sample_explanation_paths):
        explanations = group_by_pattern(
            [sample_explanation_paths[0], sample_explanation_paths[2]]
        )
        assert len(explanations) == 2
