...
class TestPredictDrugs:
    ...
    @patch('predict.torch.load')
    @patch('predict.GATLinkPredictor')
    def test_filters_existing_treats(self, mock_gat_cls, mock_torch_load, sample_triples_csv, mock_embeddings, mock_gat_model):
        mock_torch_load.side_effect = [mock_embeddings, {}]
        mock_gat_cls.return_value = mock_gat_model
        mock_gat_model.encode = MagicMock(return_value=mock_embeddings)
        ...
        results = predict_drugs(3, top_k=10, model_path=Path("dummy_model.pt"), data_dir=data_dir)
    ...
    @patch('predict.torch.load')
    @patch('predict.GATLinkPredictor')
    def test_respects_top_k(self, mock_gat_cls, mock_torch_load, sample_triples_csv, mock_embeddings, mock_gat_model):
        mock_torch_load.side_effect = [mock_embeddings, {}]
        mock_gat_cls.return_value = mock_gat_model
        mock_gat_model.encode = MagicMock(return_value=mock_embeddings)
    ...
    @patch('predict.torch.load')
    @patch('predict.GATLinkPredictor')
    def test_result_structure(self, mock_gat_cls, mock_torch_load, sample_triples_csv, mock_embeddings, mock_gat_model):
        mock_torch_load.side_effect = [mock_embeddings, {}]
        mock_gat_cls.return_value = mock_gat_model
        mock_gat_model.encode = MagicMock(return_value=mock_embeddings)
    ...
    @patch('predict.torch.load')
    @patch('predict.GATLinkPredictor')
    def test_all_results_are_drugs(self, mock_gat_cls, mock_torch_load, sample_triples_csv, mock_embeddings, mock_gat_model):
        mock_torch_load.side_effect = [mock_embeddings, {}]
        mock_gat_cls.return_value = mock_gat_model
        mock_gat_model.encode = MagicMock(return_value=mock_embeddings)
