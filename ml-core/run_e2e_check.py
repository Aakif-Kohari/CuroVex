...
def run_check(model_path: Path, data_dir: Path = None):
    ...
    for disease_mondo in test_diseases:
        ...
        try:
            ...
            disease_name = nodes_df[nodes_df['node_index'] == disease_id].iloc[0]['name']
        ...
        try:
            results = predict_drugs(disease_id, top_k=3, model_path=model_path, data_dir=data_dir)
        except Exception as e:
            print(f"Prediction failed: {e}")
            continue
            
        for res in results:
            print(f"Rank {res['rank']}: Node {res['drug_id']} ({res['drug_name']}) - Score {res['score']:.4f}")
...
def main():
    ...
    parser.add_argument("--model-path", type=str, default="artifacts/gat_link_predictor.pt", help="Path to trained GAT model")
    parser.add_argument("--data-dir", type=str, default="", help="Path to normalized data dir")
