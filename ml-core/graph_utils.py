...
def build_pyg_data(
    triples_factory: TriplesFactory, entity_embeddings: torch.Tensor
) -> Data:
...
def get_drug_disease_node_ids(nodes_path: Path) -> tuple[set[int], set[int]]:
    ...
    drugs = set(
        nodes_df[nodes_df["labels"].str.contains("Drug")]["node_index"].tolist()
    )
    diseases = set(
        nodes_df[nodes_df["labels"].str.contains("Disease")]["node_index"].tolist()
    )
