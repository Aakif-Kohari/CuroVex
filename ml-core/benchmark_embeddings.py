...
def benchmark_all(
    triples_factory: TriplesFactory, embedding_dim: int, num_epochs: int, device: str
) -> dict:
    ...
    for model_name in MODELS:
        with mlflow.start_run(run_name=f"benchmark_{model_name}"):
            ...
            mrr = result.metric_results.get_metric(
...
def save_best_embeddings(pipeline_result, output_dir: Path):
    ...
    embeddings = (
        pipeline_result.model.entity_representations[0](indices=None).detach().cpu()
    )
...
def main():
    ...
    parser.add_argument(
        "--data-dir", type=str, default="", help="Path to data directory"
    )
    ...
    print(
        f"Best model: {best_model} with MRR: {results[best_model]['metrics']['mrr']:.4f}"
    )
