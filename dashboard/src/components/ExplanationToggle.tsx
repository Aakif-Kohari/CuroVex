...
const tabs = [
  { id: 'path_based' as const, label: 'Path-Based', description: 'Meta-path reasoning' },
  { id: 'counterfactual' as const, label: 'Counterfactual', description: 'Edge-masking fidelity' },
];

export default function ExplanationToggle({ activeMethod, onToggle }: ExplanationToggleProps) {
  return (
        ...
        >
          ...
          <span
            className={`relative z-10 ${
              activeMethod === tab.id ? 'text-teal-400' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            {tab.label}
          </span>
