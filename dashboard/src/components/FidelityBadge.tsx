...
export default function FidelityBadge({ score, size = 'md' }: FidelityBadgeProps) {
  ...
  const sizeConfig = {
    sm: { container: 'w-8 h-8', text: 'text-[10px]', radius: 12, strokeWidth: 2 },
  ...
  return (
    <div className="flex items-center gap-3" title={`Fidelity: ${score.toFixed(4)} — ${label} impact`}>
      <div className={`relative ${cfg.container} flex items-center justify-center`}>
        <svg className="transform -rotate-90 w-full h-full" viewBox={`0 0 ${(cfg.radius + cfg.strokeWidth) * 2} ${(cfg.radius + cfg.strokeWidth) * 2}`}>
          <circle
      ...
      {size !== 'sm' && (
        <div>
          <p className={`${cfg.text} font-semibold ${color}`}>{label} Fidelity</p>
