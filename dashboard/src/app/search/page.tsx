    ...
    <div className="container mx-auto px-4 py-12">
      ...
      >
        <h1 className="text-3xl font-bold mb-3">
          Find Drug Repurposing <span className="text-teal-400">Candidates</span>
        </h1>
        <p className="text-slate-400 max-w-xl mx-auto">
          Search for a disease to discover potential drug candidates ranked by our
      ...
      {isLoading && (
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-sm text-slate-400">Analyzing knowledge graph...</span>
          </div>
      ...
      {isError && (
        ...
        >
          ...
          <p className="text-sm text-slate-500 max-w-md mx-auto">
            {(error as Error)?.message || 'Could not fetch predictions. Please check the API connection and try again.'}
          </p>
        </motion.div>
      )}
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-semibold">
                Results for <span className="text-teal-400">{diseaseQuery}</span>
              </h2>
              <p className="text-sm text-slate-500 mt-1">
                {predictionRun.predictions.length} candidates ranked by prediction score
              </p>
            </div>
      ...
      {!searchTrigger && !isLoading && (
        ...
        >
          ...
          <p className="text-sm text-slate-500 max-w-md mx-auto">
            Enter a disease name (e.g., &quot;Diabetes&quot;, &quot;Alzheimer&quot;) or a MONDO ID
            to discover potential drug repurposing candidates.
          </p>
        </motion.div>
      )}
    </div>
