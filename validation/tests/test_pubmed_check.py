...
def test_finds_existing_literature(
    mock_get, mock_pubmed_search_response, mock_pubmed_summary_response
):
...
@patch("validation.pubmed_check.requests.get")
def test_parses_article_details(
    mock_get, mock_pubmed_search_response, mock_pubmed_summary_response
):
    mock_search = MagicMock()
    mock_search.json.return_value = mock_pubmed_search_response
...
def test_limits_to_five_articles(mock_get):
    ...
    mock_search.json.return_value = {
        "esearchresult": {"count": "10", "idlist": ["1", "2", "3", "4", "5", "6"]}
    }
