...
| ID | User story | Priority | Status |
|---|---|---|---|
| ML-1 | As a researcher, I can benchmark TransE/RotatE/ComplEx/DistMult via PyKEEN and log results to MLflow | P0 | ✅ Done |
| ML-2 | As a researcher, I can train a GAT link-prediction model on top of the best embedding | P0 | ✅ Done |
| ML-3 | As a researcher, I can generate a ranked list of candidate drug–disease pairs for any disease node | P0 | ✅ Done |
| ML-4 | As a developer, I can export the trained model for fast inference in the API | P1 | ✅ Done |
| ML-5 | As a researcher, I can retrain on an updated graph snapshot without changing downstream code | P2 | ✅ Done |

## Epic 3 — Explainability engine
| ID | User story | Priority | Status |
|---|---|---|---|
| XAI-1 | As a researcher, I can generate meta-path explanations for a given prediction (baseline) | P0 | ✅ Done |
...
| ID | User story | Priority | Status |
|---|---|---|---|
| VAL-1 | As a researcher, I can check whether a predicted drug–disease pair has a registered clinical trial | P0 | ✅ Done |
| VAL-2 | As a researcher, I can check whether recent PubMed literature mentions a predicted pair | P1 | ✅ Done |
| VAL-3 | As a user, I can see a "supporting evidence" badge on predictions with external validation | P1 | ✅ Done |

## Epic 5 — Backend API
| ID | User story | Priority | Status |
|---|---|---|---|
| API-1 | As a frontend, I can request predictions for a disease via `GET /predictions/{disease_id}` | P0 | ✅ Done |
...
| ID | User story | Priority | Status |
|---|---|---|---|
| UI-1 | As a user, I can search for a disease and see ranked drug candidates | P0 | ✅ Done |
...
| ID | User story | Priority | Status |
|---|---|---|---|
| OPS-1 | As a developer, I can spin up the full stack locally with `docker compose up` | P0 | ✅ Done |
| OPS-2 | As a developer, CI runs lint + tests on every PR | P0 | ✅ Done |
| OPS-3 | As a developer, merges to `main` build and push Docker images | P1 | 🚧 Pending |
| OPS-4 | As a team, the demo is deployed and publicly reachable (free tiers) | P1 | 🚧 Pending |
