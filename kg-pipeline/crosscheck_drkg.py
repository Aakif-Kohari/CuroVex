...
DRKG_URL = "https://github.com/gnn4dr/DRKG/raw/master/drkg.tsv"
...
def download_drkg(output_dir: Path) -> Path:
    """Download DRKG dataset to the output directory."""
    ...
    with open(filepath, "wb") as f, tqdm(
        total=total_size, unit="iB", unit_scale=True, desc="drkg.tsv"
    ) as progress_bar:
...
def parse_drkg(filepath: Path) -> dict:
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        for line in f:
            ...
            rel_type = relation.split("::")[-1].lower() if "::" in relation else relation.lower()
...
def get_primekg_counts_from_neo4j() -> dict:
    ...
    with driver.session() as session:
        # Node counts
        node_result = session.run("MATCH (n) RETURN labels(n) AS labels, count(n) AS cnt")
        for record in node_result:
            ...
            counts["entity_counts"][label_str] = counts["entity_counts"].get(label_str, 0) + cnt
        ...
        for record in edge_result:
            r_type = record["type"]
            cnt = record["cnt"]
            counts["relation_counts"][r_type] = counts["relation_counts"].get(r_type, 0) + cnt
...
def build_report(primekg_counts: dict, drkg_counts: dict) -> str:
    ...
    for drkg_ent, cnt in drkg_counts["entity_counts"].items():
        ...
        mapped_drkg_entities[curovex_cat] = mapped_drkg_entities.get(curovex_cat, 0) + cnt
    ...
    for drkg_rel, cnt in drkg_counts["relation_counts"].items():
        ...
        mapped_drkg_relations[curovex_rel] = mapped_drkg_relations.get(curovex_rel, 0) + cnt
        
    lines.append("--- Entity/Node Counts ---")
    lines.append(f"{'Category':<18}{'PrimeKG':<11}{'DRKG':<11}{'Delta':<11}")
    
    all_ent_cats = sorted(set(primekg_counts["entity_counts"].keys()) | set(mapped_drkg_entities.keys()))
    ...
    all_rel_cats = sorted(set(primekg_counts["relation_counts"].keys()) | set(mapped_drkg_relations.keys()))
...
def main():
    parser = argparse.ArgumentParser(description="Cross-check DRKG dataset against Neo4j PrimeKG")
    parser.add_argument("--output-dir", type=str, default="data/raw", help="Directory to store DRKG dataset")
    parser.add_argument("--skip-download", action="store_true", help="Skip downloading DRKG if already present")
